...
@export var Camera_To_Follow : Node3D
...
func _ready():
	...
	_control_bitdepth.setup("Bit Depth", 2, 64, 15, 1)
	...
	_gradient.size = Vector2(get_viewport().size.x * 0.1, get_viewport().size.y)
...
func _process(delta):
	...
	if _moving_debug:
		...
		_debug_controls.position = _debug_init_pos + mouse_delta
		if Input.is_action_just_released("ui_click"):
			_moving_debug = false
	#
	#_viewport_container.material.set_shader_parameter("u_bit_depth", int(_control_bitdepth.get_value()))
	#_viewport_container.material.set_shader_parameter("u_contrast", _control_contrast.get_value())
	#_viewport_container.material.set_shader_parameter("u_offset", _control_offset.get_value())
	#_viewport_container.material.set_shader_parameter("u_dither_size", int(_control_dithersize.get_value()))
...
func _on_Move_button_down():
	_moving_debug = true
	_debug_init_pos = _debug_controls.position
...
func _on_Checkbox2_pressed():
	_viewport_container.material.set_shader_parameter("u_color_tex", palettes[1])
	_palette_preview.texture = palettes[1]

func _on_Checkbox3_pressed():
	_viewport_container.material.set_shader_parameter("u_color_tex", palettes[2])
	_palette_preview.texture = palettes[2]

func _on_Checkbox4_pressed():
	_viewport_container.material.set_shader_parameter("u_color_tex", palettes[3])
	_palette_preview.texture = palettes[3]

func _on_Checkbox5_pressed():
	_viewport_container.material.set_shader_parameter("u_color_tex", palettes[4])
...
func _on_Dither2_pressed():
	_viewport_container.material.set_shader_parameter("u_dither_tex", dither_patterns[1])
	_dither_preview.texture = dither_patterns[1]
	_dither_label.text = "Bayer 8x8"

func _on_Dither3_pressed():
	_viewport_container.material.set_shader_parameter("u_dither_tex", dither_patterns[2])
	_dither_preview.texture = dither_patterns[2]
	_dither_label.text = "Bayer 4x4"

func _on_Dither4_pressed():
	_viewport_container.material.set_shader_parameter("u_dither_tex", dither_patterns[3])
	_dither_preview.texture = dither_patterns[3]
	_dither_label.text = "Bayer 2x2"

func _on_Dither5_pressed():
	_viewport_container.material.set_shader_parameter("u_dither_tex", dither_patterns[4])
